    ...
    bool CSimpleObject::LoadFromObj(const char* fn)
    {
        ...
        else
        {
            if(Parse(fp))
            {
				...
				{
					x_min = min(x_min, m_pVertexList[i].x);
					x_max = max(x_max, m_pVertexList[i].x);
					y_min = min(y_min, m_pVertexList[i].y);
					y_max = max(y_max, m_pVertexList[i].y);
					z_min = min(z_min, m_pVertexList[i].z);
					z_max = max(z_max, m_pVertexList[i].z);
				}
				float scale = max(x_max-x_min, max(y_max-y_min, z_max-z_min));
				for ( int i=0; i<m_nVertices; ++i )
				{
					m_pVertexList[i].x = (m_pVertexList[i].x - x_min) / scale - (x_max-x_min)/(scale*2);
					m_pVertexList[i].y = (m_pVertexList[i].y - y_min) / scale - (y_max-y_min)/(scale*2);
					m_pVertexList[i].z = (m_pVertexList[i].z - z_min) / scale - (z_max-z_min)/(scale*2);
				}
